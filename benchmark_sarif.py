...
def load_rule_cwe_map(path: Optional[Path]) -> Dict[str, str]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            ...
            cwe = (row.get("cwe") or row.get("cwe_id") or "").strip().upper().replace("-", "")
            if rule_id and cwe:
                if cwe.isdigit():
                    cwe = f"CWE{cwe}"
...
def infer_cwe_from_finding(rule_id: str, message: str, rule_map: Dict[str, str]) -> Optional[str]:
    ...
    match = CWE_RE.search(rule_id) or CWE_RE.search(message)
    if match:
        return f"CWE{match.group('num')}"
        ...
        for result in run.get("results", []):
            ...
            findings.append(
                Finding(
                    ...
                    end_line=end_line,
                    snippet=snippet,
                    actual_cwe=infer_cwe_from_finding(rule_id, message, rule_map),
                )
            )
    return findings
...
def classify(regions: List[Region], points: List[ExpectedPoint], findings: List[Finding], cwe_aware: bool, window: int) -> None:
    ...
    for finding in findings:
        ...
        if safe_point:
            ...
            if cwe_aware and finding.actual_cwe and finding.actual_cwe != safe_point.cwe_id:
                finding.classification = "out_of_scope"
                continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cwe-aware", action="store_true", help="Only match findings whose CWE equals the testcase CWE.")
