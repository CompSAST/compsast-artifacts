...
2. **SonarQube Scanner**
    - Причина: не поддерживает JDK 8, при попытке сканирования возникала ошибка `Out of Memory` на сервере.
3. **Joern**
    - Причина: его набор правил (ruleset) ориентирован не на CWE, а на собственные паттерны; сканирование по CWE невозможно.

**Итог по выбору:**
- Для сканирования Juliet Java Suite использовались: OpenGrep, Semgrep, PMD, CodeQL.
- Остальные анализаторы были исключены из-за проблем с поддержкой JDK 8 или неподходящего ruleset.
...
``` bash
#!/bin/bash  
# Скрипт сканирования Juliet Java Benchmark с помощью CodeQL  
# Использование: ./scan_codeql.sh [CWE_NUMBER]  
  
set -e  
  
BENCHMARK_DIR="/mnt/c/Users/USER/Downloads/2017-10-01-juliet-test-suite-for-java-v1-3/Java"  
SRC_DIR="$BENCHMARK_DIR/src/testcases"  
SARIF_DIR="$BENCHMARK_DIR/sarif/codeql"  
CODEQL_BIN="/home/nodo/codeql/codeql/codeql"  
...
for cwe in $CWES; do  
    ...
    output="$SARIF_DIR/CodeQL__${cwe}__results.sarif"  
    if [ -f "$output" ]; then  
        echo "Already exists: $output"  
        continue  
    fi  
  
    # Оставляем только первые 5 файлов для скорости  
    cd "$dir"  
    ls *.java 2>/dev/null | tail -n +6 | xargs rm -f 2>/dev/null || true  
    rm -rf s*/ testcasesupport/ 2>/dev/null || true  
    cd -  
  
    db_name="codeql_db_cwe$cwe"  
    rm -rf "$db_name"  
  
    $CODEQL_BIN database create --language=java --source-root="$dir" \  
        --build-mode=none "$db_name" 2>&1 | tail -1  
  
    if [ -d "$db_name" ]; then  
        $CODEQL_BIN database analyze "$db_name" \  
            --format=sarifv2.1.0 \  
            --output="$output" \  
            /home/nodo/.codeql/packages/codeql/java-queries/1.11.0/Security/CWE/CWE-798/ 2>&1 | tail -1  
